...
class RegTrainer:
    def __init__(self, config_name=''):
        ...
        self.set_loaders()
        self.best_acc = 1e8 
        self.ssim_loss = monai.losses.ssim_loss.SSIMLoss(spatial_dims=2, data_range=2)
    
    def reg_criterion(self, x_native, x_predict, config):
        
        mae = F.l1_loss(x_native, x_predict)
        ssim = self.ssim_loss(x_native+1., x_predict+1.)
        loss = mae
        config['ssim'].append(ssim.item())
        config['mae'].append(mae.item()*1000.)
    ...
    def test(self, i_epoch):
        ...
        for batch_idx, (native, contrast) in loop:
            ...
            output = self.net(contrast)
            loss = self.reg_criterion(native, output, config_log)
            
            # LOOPA and PUPA
            loop.set_description(f"Epoch (Test)[{i_epoch}/{self.num_epochs}]")
            loop.set_postfix(loss=np.mean(config_log['loss']), mae=np.mean(config_log['mae']))
    ...
    def wandb_log_nums(self, config, name):
        log_data = {
            ...
            f'{name} ssim': np.mean(config['ssim']),
            f'{name} mae': np.mean(config['mae']),
