...
class FLowTrainer:
    ...
    def flow_matching_criterion(self, x_native, x_contrast, config):
        ...
        if self.config['net_name'] == 'TimeResNet' or self.config['net_name'] == 'TimeResNet_wo_attention':
    ...
    def model_selection(self, metrics):
        if np.mean(metrics['loss']) < self.best_acc:
            ...
            self.save_model(
                metrics, metrics['i_epoch'], self.model_name + f'_best')
        else:
            self.save_model(
                metrics, metrics['i_epoch'], self.model_name + f'_last')
    ...
    def save_model(self, metrics, i_epoch, name):
        ...
        print(
            f'Saved!:) Epoch[{i_epoch}], loss = {np.mean(metrics["loss"]):.04f}')
    ...
    def set_loaders(self):
        ...
        transform_train = get_transform_train(
            self.save_config['transform_train'])
    ...
    def load(self, filename, config_name=None):
        checkpoint = torch.load(
            f'./checkpoint/{filename}.pth', map_location='cpu', weights_only=False)
        self.set_net()
    ...
    def set_wandb(self):
        ...
        if self.wandb_use:
            ...
            self.run = wandb.init(
                project=self.config['project'], config=self.save_config)
